...
# Spatio-Temporal Predictive Transformer (STP-TranAD)
# Refactored for Predicive Anomaly Detection with Patching and Branching
class STP_TranAD(nn.Module):
	def __init__(self, feats):
		super(STP_TranAD, self).__init__()
		self.name = 'STP_TranAD'
		self.lr = lr
		self.batch = 64
		self.n_feats = feats
		self.n_window = 100 # Enlarged for patching
		self.patch_size = 5
		self.n_patches = self.n_window // self.patch_size
		self.d_model = 64
		self.d_feat = 16 
		self.H = 10 # Forecast horizon (seconds)

		# 1. Patching & Projection
		self.patch_proj = nn.Linear(self.patch_size, self.d_feat)
		
		# 2. Spatial Topology Learning (Replaced DGL GAT with pure PyTorch equivalents)
		# A GAT over a fully connected graph is mathematically equivalent to Self-Attention
		self.gat = nn.MultiheadAttention(embed_dim=self.d_feat, num_heads=1, batch_first=True)
		self.feat_merge = nn.Linear(feats * self.d_feat, self.d_model)

		# 3. Transformer Encoder (Temporal Modeling)
		self.pos_encoder = PositionalEncoding(self.d_model, 0.1, self.n_patches)
		# Custom encoder layers to expose attention weights if needed, 
		# for now using standard layers and we'll compute Prior locally if possible
		encoder_layers = nn.TransformerEncoderLayer(d_model=self.d_model, nhead=8, dim_feedforward=256, dropout=0.1)
		self.transformer_encoder = nn.TransformerEncoder(encoder_layers, 2)

		# 4. Decoders (Branch A: Adversarial Reconstruction of Past)
		decoder_layers1 = nn.TransformerDecoderLayer(d_model=self.d_model, nhead=8, dim_feedforward=256, dropout=0.1)
		self.decoder1 = nn.TransformerDecoder(decoder_layers1, 1)
		decoder_layers2 = nn.TransformerDecoderLayer(d_model=self.d_model, nhead=8, dim_feedforward=256, dropout=0.1)
		self.decoder2 = nn.TransformerDecoder(decoder_layers2, 1)
		self.recon_fcn = nn.Sequential(nn.Linear(self.d_model, feats), nn.Sigmoid())

		# 5. Predictive Head (Branch B: Stochastic Forecaster)
		self.forecaster = nn.Sequential(
			nn.Linear(self.d_model, 128),
			nn.ReLU(),
			nn.Linear(128, feats * self.H),
			nn.Sigmoid()
		)

		# 6. Learnable Multi-task Loss (Option B)
		self.log_vars = nn.Parameter(torch.zeros(3)) # [Recon, Prior, Forecast]

	def encode(self, src):
		seq_len, batch_size, n_feats = src.shape
		# Patching: from (Seq, Batch, Feats) to (Batch, N_patches, Patch_size, Feats)
		x = src.permute(1, 0, 2)
		x = x.reshape(batch_size, self.n_patches, self.patch_size, self.n_feats)
		# Permute to apply linear projection on patch_size dim: (Batch, N_patches, Feats, Patch_size)
		x = x.permute(0, 1, 3, 2)
		x = self.patch_proj(x) # (Batch, N_patches, Feats, D_feat)
		
		# Spatial Topology (Pure PyTorch Self-Attention across features)
		x_flat = x.view(-1, self.n_feats, self.d_feat) # (Batch * N_patches, Feats, D_feat)
		x_gat, _ = self.gat(x_flat, x_flat, x_flat)
		x_gat = x_gat.reshape(batch_size, self.n_patches, -1)
		x_merged = self.feat_merge(x_gat)
		x_merged = x_merged.permute(1, 0, 2) # (N_patches, Batch, D_model)
		x_merged = self.pos_encoder(x_merged)
		memory = self.transformer_encoder(x_merged)
		return memory
	...
	def forward(self, src):
		...
		# Branch A: Reconstruction
		tgt = memory[-1:].repeat(1, 1, 1) # Use last patch for reconstruction of the final state
		x1 = self.recon_fcn(self.decoder1(tgt, memory))
		
		# Phase 2 refiner (adversarial logic)
		# We'll use a simpler conditioning for the hybrid model: 
		# Inject error into the memory or tgt for the second decoder
		x2 = self.recon_fcn(self.decoder2(tgt, memory))
		
		# Branch B: Forecasting
		forecast = self.forecaster(memory[-1:]) # (1, Batch, Feats*H)
		forecast = forecast.view(1, -1, self.H, self.n_feats) # (1, Batch, H, Feats)
		forecast = forecast.squeeze(0).permute(1, 0, 2) # (H, Batch, Feats)
